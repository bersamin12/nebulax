...
The app opens on the **Overview** page: what the Train Digital Twin is, how each PS3 model works (with its architecture diagram and the literature behind it), the EDA, the ablation ladders, the two exploratory systems and the team. **Open Digital Twin** in the header or the hero leads to the predict page (`?page=predict`); **Start tutorial** opens it with a guided tour that spotlights one control at a time (it also runs by itself on a first visit, and `?page=predict&tour=1` links to it). The control inside the spotlight stays live, so a reader can pick a system, drop files or press RUN while the tour explains it. On the predict page every explanation variable has an (i) icon with its definition.
...
The site is built for desktop browsers. The console is a fixed 1440 by 900 layout scaled to fill the window, up on a 2.5K or 4K screen (the 3D view is drawn at the matching pixel ratio, so it stays sharp) and down to 60 percent on a small one, below which the page scrolls. Phones and tablets get the same console under a "limited support" notice, fitted to the screen.

On the Digital Twin page all six systems are listed as compact tiles (the four models with their CV score, the two exploratory systems marked as such); clicking a tile selects it and shows its description, clicking it again hides the description. Choosing files opens a check dialog that lists the columns each file carries against what the model expects (Door: the required Datetime, Motor current and Door leaf position columns; Rail: 129 columns; SHM: a headerless single column; ACV: the server's workbook report) and queues only the files that match.
