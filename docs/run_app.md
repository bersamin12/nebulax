...
The app can start without the released datasets. Upload the organiser Test files through the **Predict** page, or use compatible files with the required schemas. Use **Door** for one door stream CSV, **ACV** for an Excel case workbook, **Rail** for rail CSV recordings, and **SHM** for headerless stress CSVs. Choose a task, select or drop its files, press **RUN**, inspect the explanations and download the generated CSV. The Rail folder can contain many files; the page sends batches and the server scores them sequentially.
